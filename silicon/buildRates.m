...
baseDir = '/Volumes/MONOLITH/Silicon/'
behavDir = '/Volumes/MONOLITH/Silicon/Behavior/SoloArrays/'
...
for i = 1:length(d1)
    ...
    for j=1:length(d2)
        ...
        for k=1:length(d3)
            ...
            summarizeRates(eval(d3(k).name(8:13)), str2num(d3(k).name(13)), d21, clustparam,b)
            eval('clear shank*');
