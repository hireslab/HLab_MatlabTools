...
% subplot(1,4,1)
% hold on
% dColor=jet(70)%double(uint8(10*log2(max(cellfun(@(x)nanmean(x),Stest.baselineSR))))+1));
% colormap(dColor(1:64,:));
% 
% for cNum = 1:148
%     plot(Stest.depth{cNum}, Stest.Vmod{cNum}(1),'ko','MarkerFaceColor',dColor(uint8(10*log2(nanmean(Stest.baselineSR{cNum})))+1,:))
% end
...
% hold on
% for cNum = 1:148
%         plot(Stest.depth{cNum}, Stest.Vmod{cNum}(2),'ko','MarkerFaceColor',dColor(uint8(10*log2(nanmean(Stest.baselineSR{cNum})))+1,:))
% end
% axis([0 700 -9 9])
...
% 
% for cNum = 1:148
%         plot(Stest.depth{cNum}, Stest.Vmod{cNum}(3),'ko','MarkerFaceColor',dColor(uint8(10*log2(nanmean(Stest.baselineSR{cNum})))+1,:))
% end
% axis([0 700 -1.1 1.1])
% grid on
