function summarizeSpikesbak(array, contacts, params)

contactAlignment=cellfun(@(x)x.segmentInds{1},contacts,'UniformOutput',0);

contactAlignVect=repmat(params.meanAnswerTime,length(array.trials),1);

for i=1:length(array.trials);
    if numel(contactAlignment{i});
    tmp=find(array.trials{i}.whiskerTrial.time{1}(contactAlignment{i}(:,1))>.78...
        & array.trials{i}.whiskerTrial.time{1}(contactAlignment{i}(:,1)) < array.trials{i}.pinAscentOnsetTime);
    
    if numel(tmp)
        contactAlignVect(i)=array.trials{i}.whiskerTrial.time{1}(contactAlignment{i}(tmp(1),1));
    end
    end
...
for i=1:5
    ...
    if sum(cellfun(@(x)x.trialContactType,contacts,'UniformOutput',1)==(i-1))
        ...
        array.plot_lick_raster(array.trialNums(intersect(find(cellfun(@(x)x.trialContactType,contacts,'UniformOutput',1)==(i-1)),useTrials)),...
            'Sequential',contactAlignVect(intersect(find(cellfun(@(x)x.trialContactType,contacts,'UniformOutput',1)==(i-1)),useTrials)),'lines');
        hold on;
        array.plot_spike_raster(array.trialNums(intersect(find(cellfun(@(x)x.trialContactType,contacts,'UniformOutput',1)==(i-1)),useTrials)),...
            'Sequential',contactAlignVect(intersect(find(cellfun(@(x)x.trialContactType,contacts,'UniformOutput',1)==(i-1)),useTrials)),'lines');
        end
    axis([-.2 .5 0 ymax]);
    title(touchTitles{i});
