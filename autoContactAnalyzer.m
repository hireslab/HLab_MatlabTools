...
if nargin==1
    ...
    g=struct;
    g.touchThresh = [.2 .2 .3 .3]; %Touch threshold for go (protraction, retraction), no-go (protraction,retraction). Check with Parameter Estimation cell
    g.goProThresh = -10; % Mean curvature above this value indicates probable go protraction, below it, a go retraction trial.
    g.nogoProThresh = -15; % Mean curvature above this value indicates probable nogo protraction, below it, a nogo retraction trial.
    g.poleOffset = .768; % Time where pole becomes accessible
    g.poleEndOffset = .3; % Time between start of pole exit and inaccessiblity
    g.spikeSynapticOffset = .02; % Estimated synaptic delay for assigning spikes to contact epochs
    g.tid=0; % Trajectory id
    g.framesUsed=1:length(array.trials{find(array.whiskerTrialInds,1)}.whiskerTrial.time{1});
    g.curveMultiplier=1;
    disp(g)
...
for k = find(array.whiskerTrialInds)
    ...
    if max(cat(1,array.missTrialNums(:),array.hitTrialNums(:))==array.trialNums(k))  %determine trial type
        touchThreshi=max(g.touchThresh(1:2));     % go trials
    else
        touchThreshi=max(g.touchThresh(3:4));     % nogo trials
    end

    % Finds indexes of time periods of contacts by distance to pole - curvature
    contacts{k}.contactInds{1} = array.trials{k}.whiskerTrial.time{1}(array.trials{k}.whiskerTrial.distanceToPoleCenter{1} - ...
        g.curveMultiplier*abs(array.trials{k}.whiskerTrial.kappa{1}) < touchThreshi);  
    
    % Crops indexs to only pole available times
    contacts{k}.contactInds{1}=contacts{k}.contactInds{1}(contacts{k}.contactInds{1} >...
        g.poleOffset+array.trials{k}.behavTrial.pinDescentOnsetTime & contacts{k}.contactInds{1} < g.poleEndOffset +array.trials{k}.behavTrial.pinAscentOnsetTime );

    [~,~,contacts{k}.contactInds{1}]=intersect(contacts{k}.contactInds{1},array.trials{k}.whiskerTrial.time{1});

    % Calculate the mean curvature during contact period
    meanContactCurve(k)=nanmean(array.trials{k}.whiskerTrial.thetaAtBase{1}(contacts{k}.contactInds{1})-array.trials{k}.whiskerTrial.thetaAtContact{1}(contacts{k}.contactInds{1}));
...
for k = find(array.whiskerTrialInds)
    ...
    if max(cat(1,array.missTrialNums(:),array.hitTrialNums(:))==array.trialNums(k))
        if meanContactCurve(k)>g.goProThresh
            touchThreshi=g.touchThresh(1); % use go / protraction threshold
            trialContactType(k)=1;
        else
            touchThreshi=g.touchThresh(2); % use go / retraction threshold
            trialContactType(k)=2;
        end
        
    else
        if meanContactCurve(k)>g.nogoProThresh
            touchThreshi=g.touchThresh(3); % use no-go / protraction threshold
            trialContactType(k)=3;
        else
            touchThreshi=g.touchThresh(4); % use no-go / retraction threshold
    ...
    contacts{k}.contactInds{1} = ...
        contacts{k}.contactInds{1}(contacts{k}.contactInds{1} > g.poleOffset+array.trials{k}.behavTrial.pinDescentOnsetTime ...
        & contacts{k}.contactInds{1} < g.poleEndOffset+array.trials{k}.behavTrial.pinAscentOnsetTime );

    [~,~,contacts{k}.contactInds{1}]=intersect(contacts{k}.contactInds{1},array.trials{k}.whiskerTrial.time{1}); % c, ia are unused
...
for k=find(array.whiskerTrialInds)
    if isempty(contacts{k}.segmentInds{1})==0
        for i=1:length(contacts{k}.segmentInds{1}(:,1));
            contacts{k}.meanM0{1}(i)=nanmean(array.trials{k}.whiskerTrial.M0{1}(contacts{k}.segmentInds{1}(i,1):contacts{k}.segmentInds{1}(i,2)));
        end
    else
...
for k=find(array.whiskerTrialInds)
    if isempty(contacts{k}.segmentInds{1})==0
        for i=1:length(contacts{k}.segmentInds{1}(:,1));
            contacts{k}.peakM0{1}(i)=max(abs(array.trials{k}.whiskerTrial.M0{1}(contacts{k}.segmentInds{1}(i,1):contacts{k}.segmentInds{1}(i,2))))*...
...
for k=find(array.whiskerTrialInds)
    if isempty(contacts{k}.segmentInds{1})==0
        for i=1:length(contacts{k}.segmentInds{1}(:,1));
            lim=array.trials{k}.whiskerTrial.framePeriodInSec*contacts{k}.segmentInds{1}(i,:)+g.spikeSynapticOffset;
...
for k=find(array.whiskerTrialInds)
    if isempty(contacts{k}.segmentInds{1})==0
        contacts{k}.contactLength{1}=array.trials{k}.whiskerTrial.time{1}(contacts{k}.segmentInds{1}(:,2))-array.trials{k}.whiskerTrial.time{1}(contacts{k}.segmentInds{1}(:,1));
    else
...
for k=find(array.whiskerTrialInds);
    contacts{k}.M0combo{1}=array.trials{k}.whiskerTrial.M0I{1};
    contacts{k}.M0combo{1}(find(abs(contacts{k}.M0combo{1})>1e-7))=NaN;

    contacts{k}.FaxialAdj{1}=zeros(1,length(array.trials{k}.whiskerTrial.Faxial{1}));
    ...
    if isnan(contacts{k}.contactInds{1})==0
        ...
        contacts{k}.M0combo{1}(ind)=array.trials{k}.whiskerTrial.M0{1}(ind);   % build M0Combo from the Segment Inds that have had the 1-2 frame drops filtered out
        contacts{k}.FaxialAdj{1}(ind)=array.trials{k}.whiskerTrial.Faxial{1}(ind);
...
% Plot the estimated primary contact type by trial number
    ...
    subplot(2,3,[2 3]);
    plot(array.whiskerTrialNums,meanContactCurve(find(array.whiskerTrialInds)),'.');
    xlabel('Trial Number');
    ylabel('Mean Contact Curvature (\kappa)');
    grid on
    
    subplot(2,3,[5 6]);
    plot(array.whiskerTrialNums,cellfun(@(x)x.trialContactType,contacts(array.whiskerTrialInds)),'.') 
    ...
    plot([0 1],[0 1],'.');
    set(gca,'Visible','off');
    text(-.3,.9, ['\fontsize{8}' 'Trajectory ID :\bf ' num2str(g.tid)]);
    text(-.3,.8, ['\fontsize{8}' 'Pole Delay Offset  On :\bf ' num2str(g.poleOffset) '\rm  Off : \bf' num2str(g.poleEndOffset) '(s)']);
    text(-.3,.7, ['\fontsize{8}' 'Pro/Ret Threshold  Go :\bf ' num2str(g.goProThresh) '\rm  NoGo : \bf' num2str(g.nogoProThresh)]);
    text(-.3,.6, ['\fontsize{8}' 'Contact Threshold :\bf ' num2str(g.touchThresh(1)) ' / ' num2str(g.touchThresh(2)) ' / ' num2str(g.touchThresh(3)) ' / ' num2str(g.touchThresh(4))]);
    text(-.3,.5, ['\fontsize{8}' 'Curvature Multiplier :\bf ' num2str(g.curveMultiplier)]);
...
%     if isempty(tmp)==0
%         plot(array.trials{tmp(1)}.whiskerTrial.time{1},array.trials{tmp(1)}.whiskerTrial.distanceToPoleCenter{1},'.-')
...
%             ind=find(round(10000*array.trials{k}.whiskerTrial.time{1})>=lim(1) & round(10000*array.trials{k}.whiskerTrial.time{1})<lim(2));
%             if isempty(ind)==0;
%                 ind2=round((array.trials{k}.whiskerTrial.time{1}(ind)-array.trials{k}.whiskerTrial.time{1}(ind(1)))*1000)+1;
%             else
%                 ind2=[];
%             end
%             
...
%             spikeTriggerPosition(ind2,i)=array.trials{k}.whiskerTrial.theta{1}(ind);
% 
%             v=array.trials{k}.whiskerTrial.get_velocity(p.tid,3);
%             spikeTriggerVelocity(ind2,i)=v(ind);
% 
%             a=array.trials{k}.whiskerTrial.get_acceleration(p.tid,3);
...
% for k=find(array.whiskerTrialInds);
%     ind=find(array.trials{k}.whiskerTrial.time{1} < array.trials{k}.behavTrial.answerPeriodTime(2)+1);
%     decisionAlignedM0Combo{k}=zeros(maxTime,1);
%     decisionAlignedM0Combo{k}(round((array.trials{k}.whiskerTrial.time{1}(ind)-array.trials{k}.whiskerTrial.time{1}(ind(end)))*1000)+4500)=M0combo{k}(ind);
