% trialContactBrowserMulti is a tool to determine and examine whisking and contacts
%
% trialContactBrowserMulti(array) will construct an first pass estimate of
% contacts using default parameters.
%
% trialContactBrowserMulti(array, contacts) will load the browser with the
% contact info specified in the contacts structure, but default detection
% parameters.
%
% trialContactBrowserMulti(array, contacts, params) will load the browser with
...
function trialContactBrowserMulti(array, contacts, params, varargin)
if nargin <4
    if nargin==1
        disp('Contacts data missing, building and assigning to workspace as "contacts"')
        [contacts, params]=autoContactAnalyzerMulti(array);
    ...
    set(fbutton,'ClickedCallback',['trialContactBrowserMulti(' params.arrayname ',' params.contactsname ', params,''next'')'])
    set(bbutton,'ClickedCallback',['trialContactBrowserMulti(' params.arrayname ',' params.contactsname ', params,''last'')'])
    set(abutton,'ClickedCallback',['trialContactBrowserMulti(' params.arrayname ',' params.contactsname ', params,''add'')'])
    set(dbutton,'ClickedCallback',['trialContactBrowserMulti(' params.arrayname ',' params.contactsname ', params,''del'')'])
    set(rbutton,'ClickedCallback',['trialContactBrowserMulti(' params.arrayname ',' params.contactsname ', params,''recalc'')'])
    set(sbutton,'ClickedCallback',['trialContactBrowserMulti(' params.arrayname ',' params.contactsname ', params,''save'')'])
    
    set(floatToggle,'OnCallback',  ['trialContactBrowserMulti(' params.arrayname ',' params.contactsname ', params,''floatOn'')'])
    set(floatToggle,'OffCallback', ['trialContactBrowserMulti(' params.arrayname ',' params.contactsname ', params,''floatOff'')'])
    set(zoomToggle,'OnCallback',   ['trialContactBrowserMulti(' params.arrayname ',' params.contactsname ', params,''zoomOut'')'])
    set(zoomToggle,'OffCallback',  ['trialContactBrowserMulti(' params.arrayname ',' params.contactsname ', params,''zoomIn'')'])
    
    
    % Setup menus
    m1=uimenu(hParamBrowserGui,'Label','Time Period','Separator','on');
    uimenu(m1,'Label','All'                          ,'Callback', ['trialContactBrowserMulti(' params.arrayname ',' params.contactsname ', params,''all'')']);
    uimenu(m1,'Label','Contacts Only'                ,'Callback', ['trialContactBrowserMulti(' params.arrayname ',' params.contactsname ', params,''contactsOnly'')']);
    uimenu(m1,'Label','Exclude Contacts'             ,'Callback', ['trialContactBrowserMulti(' params.arrayname ',' params.contactsname ', params,''excludeContacts'')']);
    uimenu(m1,'Label','Pole Presentation to Decision','Callback', ['trialContactBrowserMulti(' params.arrayname ',' params.contactsname ', params,''poleToDecision'')']);
    uimenu(m1,'Label','First Contact to Decision'    ,'Callback', ['trialContactBrowserMulti(' params.arrayname ',' params.contactsname ', params,''contactToDecision'')']);
    uimenu(m1,'Label','Post Decision'                ,'Callback', ['trialContactBrowserMulti(' params.arrayname ',' params.contactsname ', params,''postDecision'')']);
    uimenu(m1,'Label','Post Pole'                    ,'Callback', ['trialContactBrowserMulti(' params.arrayname ',' params.contactsname ', params,''postPole'')']);
    uimenu(m1,'Label','Abritrary Range'              ,'Callback', ['trialContactBrowserMulti(' params.arrayname ',' params.contactsname ', params,''arbitrary'')']);
    
    m2=uimenu(hParamBrowserGui,'Label','Adjust parameters','Separator','on');
    uimenu(m2,'Label','Plots'   ,'Callback',['trialContactBrowserMulti(' params.arrayname ',' params.contactsname ', params,''adjPlots'')']);
    uimenu(m2,'Label','Spikes'  ,'Callback',['trialContactBrowserMulti(' params.arrayname ',' params.contactsname ', params,''adjSpikes'')']);
    uimenu(m2,'Label','Contacts','Callback',['trialContactBrowserMulti(' params.arrayname ',' params.contactsname ', params,''adjContacts'')']);
    uimenu(m2,'Label','Trial Range','Callback',['trialContactBrowserMulti(' params.arrayname ',' params.contactsname ', params,''adjTrials'')']);
    
    uimenu(hParamBrowserGui,'Label','Jump to sweep','Separator','on','Callback',['trialContactBrowserMulti(' params.arrayname ',' params.contactsname ', params,''jumpToSweep'')']);
    ...
    %     uimenu(m3,'Label','STA'     ,'Callback',['trialContactBrowserMulti(' params.arrayname ',' params.contactsname ', params,''STA'')']);
    %     uimenu(m3,'Label','Clusts'  ,'Callback',['trialContactBrowserMulti(' params.arrayname ',' params.contactsname ', params,''clusts'')']);
    %     uimenu(m3,'Label','Tuning'  ,'Callback',['trialContactBrowserMulti(' params.arrayname ',' params.contactsname ', params,''tuning'')']);
    %     uimenu(m3,'Label','Contacts','Callback',['trialContactBrowserMulti(' params.arrayname ',' params.contactsname ', params,''contacts'')']);
    %     uimenu(m3,'Label','Fit'     ,'Callback',['trialContactBrowserMulti(' params.arrayname ',' params.contactsname ', params,''fit'')']);
    %
    %
    ...
    for j = 1:length(varargin);
        ...
        switch argString
            ...
            case 'recalc'
                [contacts params] = autoContactAnalyzerMulti(array, params, contacts, 'recalc');
            ...
            case 'adjContacts'
                ...
                if ~isempty(dlgout)
                    ...
                    [contacts, params]=autoContactAnalyzerMulti(array, params, contacts);
                    setappdata(hParamBrowserGui,'contacts',contacts);
                    setappdata(hParamBrowserGui,'params',params);
...
time=array.trials{params.sweepNum}.whiskerTrial.time{tidx}; % All times in current trial
...
switch params.displayType
    ...
    case 'contactsOnly'
        params.framesUsed = contacts{params.sweepNum}.contactInds{tidx};
        
    case 'excludeContacts'
        params.framesUsed = ones(size(time));
        params.framesUsed(contacts{params.sweepNum}.contactInds{tidx})=0;
    ...
    case 'contactToDecision'
        if isempty(contacts{params.sweepNum}.contactInds{tidx})==0
            params.framesUsed = find(time > time(contacts{params.sweepNum}.contactInds{tidx}(1)) &...
                time < params.meanAnswerTime);
        else
            params.framesUsed=[];
        end
...
if isempty(cW)==1
    ...
    params.cropind=find(cW.time{tidx} > params.poleOffset+cB.pinDescentOnsetTime & cW.time{tidx} < params.poleEndOffset+cB.pinAscentOnsetTime);
    cind=contacts{params.sweepNum}.contactInds{tidx};
    y1=cW.distanceToPoleCenter{tidx}(params.cropind);
    x1=cW.kappa{tidx}(params.cropind);
    y2=cW.distanceToPoleCenter{tidx}(cind);
    x2=cW.kappa{tidx}(cind);
    tmax=max(cW.time{tidx});
    ...
    %    text(.1,.9, ['\fontsize{10}' 'Analysis Time Period : ' params.displayType '  ' num2str([params.arbTimes{tidx} params.arbTimes{2}])]);
%    text(.1,.8, ['\fontsize{10}' 'Spike Synaptic Offset : ' num2str(params.spikeSynapticOffset) ' (s)']);
    ...
    subplot(4,3,[1 2  4 5]);
    hold off;
    plot(cW.time{tidx},cW.distanceToPoleCenter{tidx},'.-k','Tag','t_d')
    hold on
    set(gca,'XLim',[.5 2],'YLim',[-.25 1]);
    
    plot(cW.time{tidx}(cind),cW.distanceToPoleCenter{tidx}(cind),'.r')
    ...
    plot(cW.time{tidx},cW.deltaKappa{tidx},'.-k')
    hold on
    
    plot(cW.time{tidx}(cind),cW.deltaKappa{tidx}(cind),'.r')
    ...
    if ~isfield(params,'floatingBaseline')
        plot(array.trials{params.sweepNum}.whiskerTrial.time{tidx},contacts{params.sweepNum}.M0combo{tidx},'-w.','MarkerSize',6)
        plot(array.trials{params.sweepNum}.whiskerTrial.time{tidx}(cind),cW.M0{tidx}(cind),'r.','MarkerSize',8)
    ...
    elseif params.floatingBaseline
        ...
        plot(array.trials{params.sweepNum}.whiskerTrial.time{tidx},contacts{params.sweepNum}.M0comboAdj{tidx},'-w.','MarkerSize',6)
        plot(array.trials{params.sweepNum}.whiskerTrial.time{tidx}(cind),contacts{params.sweepNum}.M0comboAdj{tidx}(cind),'r.','MarkerSize',8)
...
%
%     cla;
%     plot(array.trials{params.sweepNum}.whiskerTrial.time{tidx},M0combo,'-k.')
%     title(strcat('Forces associated with trial #',num2str(params.trialNums(params.sweepNum))))
%     hold on;
%     plot(array.trials{params.sweepNum}.whiskerTrial.time{tidx}(cind),cW.M0{tidx}(cind),'.r')
