...
gtind = bitand(bitand(tind,T.hitTrialInds),T.missTrialInds);
ntind = bitand(bitand(tind,T.hitTrialInds),T.missTrialInds);
...
plot(U.(cellnames{i}).C.decision.peakM0(tind,1),V(tind),'.','color',colorind(c,:));
end
