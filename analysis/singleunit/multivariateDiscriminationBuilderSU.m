...
%Snums_L4  = intersect(find([SU.dist{:}] < .250), find(cellfun(@(x)x(3),SU.recordingLocation) > .418 & cellfun(@(x)x(3),S.recordingLocation) < .588));

for cellNum = 1:52%Snums_L4


[CA, T, contacts, params] = loadSUData(cellNum, SU);

DA = struct;
time = cell(length(T.trials),1);
DA.recordinglocation = SU.recordingLocation{cellNum};
...
for i = whiskerTIN%intersect(useFlag, whiskerTIN)
    ...
    DA.spikeTimes{i} = round(double(T.trials{i}.spikesTrial.spikeTimes)/10-1000*wTTO); % Spiketimes in this trial, aligned to video start
    
    time{i}  = T.trials{i}.whiskerTrial.time{1}; % Placeholder for time values of this trial
    DA.samplingPeriodIdx{i} = time{i} <= min([DA.samplePeriodCutoff+T.trials{i}.pinDescentOnsetTime DA.firstLick{i} T.trials{i}.pinAscentOnsetTime]) & ...
...
save(['z:\users\Andrew\Whisker Project\SingleUnit\DiscrimAnalysisArrays\DA_' num2str(cellNum) '_' SU.trialArrayName{cellNum}(13:end)], 'DA')
end
