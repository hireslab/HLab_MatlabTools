...
for cellNum = 1:length(Snums)
    ...
    sfS = T.trials{find(T.whiskerTrialInds,1,'first')}.shanksTrial.sampleRate;
    wTTO       = T.whiskerTrialTimeOffset;
    spikeTimes = cellfun(@(x)x.shanksTrial.clustData{clustInd}.spikeTimes,T.trials,'UniformOutput',0);
    ...
    for i = 1:length(perfT);
        ...
        sweepLength = T.trials{perfT(i)}.shanksTrial.sweepLengthInSamples / sfS * 1000;
        theta{perfT(i)} = cW.thetaAtBase{1}(~isnan(cW.thetaAtBase{1}));
        time{perfT(i)} = cW.time{1}(~isnan(cW.thetaAtBase{1}));
        cS = T.trials{perfT(i)}.shanksTrial.clustData{clustInd};
    ...
    for j = 1:nbins
        phaseSRMean(j) = nanmean(phaseSR{j});
        phaseBinMean(j) = nanmean(phaseBin{j});
    end
    shuffledPhaseSRMean = {};
    ...
    title(S.filename{cellNum}(1:end-4))
    ...
    print(gcf, '-depsc', [savedir 'PhaseSR_' S.filename{cellNum}(1:end-4)]);
    ...
    CA.phaseSRBinned = phaseSRBinned;
    CA.hh                = hh;
    ...
    save(['Z:\users\Andrew\Whisker Project\Silicon\CellAnalysisArrays\' S.filename{cellNum}(1:end-4)], 'CA')
