...
for cnum = 1:148;
    ...
    [CA, T, contacts, DA, clustInd, params] = loadSiData(cnum, S);
    
    clustInd       = intersect(find(T.cellNum == S.clust{cnum}), find(T.shankNum == S.shank{cnum}));
    
    synapticDelay   = 10;
    
    TC.useFlag      = intersect(find(cellfun(@(x)~isempty(x),DA.samplingPeriodinMs)),find(cellfun(@(x)x.shanksTrial.clustData{clustInd}.useFlag,T.trials)));
    ...
    for i = TC.useFlag
        ...
        if ~isempty(contacts{i}.contactInds{1}(contacts{i}.contactInds{1}<DA.samplingPeriodinMs{i}(end)))
...
%TC.contactIdx{i}  = 
    ...
    subplot(4,3,[1:3]);
    T.plot_spike_raster(TC.useFlag,clustInd);
    set(gca,'xlim',[0 3])
    subplot(4,3,4)
    text(0, 1, [S.anm{cnum} S.session{cnum}])
    text(0, .85, ['Si Cell #' num2str(cnum)])
    text(0, .7, ['shank ' num2str(S.shank{cnum})])
    text(0, .55, ['clust ' num2str(S.shank{cnum}) '(' num2str(clustInd) ')'])
    text(0, .4, ['Loc ' sprintf('%0.3g',S.recordingLocation{cnum}(1)) ',' sprintf('%0.3g',S.recordingLocation{cnum}(2)) ',' sprintf('%0.3g',S.recordingLocation{cnum}(3)) ])
    set(gca,'visible','off')
    for i = 1:8
    ...
    save(['Z:\users\Andrew\Whisker Project\Silicon\TuningCurveArrays\TC-S' num2str(cnum)], 'TC')
    print(gcf, '-depsc',  ['z:\users\Andrew\Whisker Project\Silicon\Figures\TuningCurves\S' num2str(cnum) '-' S.filename{cnum}(1:end-4)])
end
%
%
%
