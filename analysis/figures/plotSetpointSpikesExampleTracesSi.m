...
for i = 1:20
    ...
    try
    plot(T.trials{tct(i)}.whiskerTrial.time{1}(~isnan(cW.thetaAtBase{1}))-timeshift,setpoint+yshift,'.',...
...
title(['Setpoint, Spikes & Licks ' T.mouseName ' ' num2str(T.shankNum(cellID)) '-' num2str(T.cellNum(cellID))...
    ' (20 Example Trials)'])

set(gca,'XLim',[-1 1],'YLim', [0 1.05]*yshift,'Color','w');
xlabel('Time from amplitude threshold crossing (s)')
ylabel('Setpoint (N * m)')
print('-depsc', ['Z:\users\Andrew\Whisker Project\Figures\ExampleTraces\SetpointSpikes_' T.mouseName '_Cell_'...
