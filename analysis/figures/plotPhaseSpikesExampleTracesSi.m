...
for i = 1:10
    ...
    try
    plot(T.trials{tct(i)}.whiskerTrial.time{1}(~isnan(cW.thetaAtBase{1}))-timeshift,phase+yshift,'.',...
...
title(['Phase, Spikes & Licks ' T.mouseName ' ' num2str(T.shankNum(cellID)) '-' num2str(T.cellNum(cellID))...
    ' (20 Example Trials)'])

set(gca,'XLim',[-0.1 .3],'YLim', [0 1.05]*yshift,'Color','w');
xlabel('Time from first contact (s)')
ylabel('Phase (N * m)')
print('-depsc', ['Z:\users\Andrew\Whisker Project\Figures\ExampleTraces\PhaseSpikes_' T.mouseName '_Cell_'...
